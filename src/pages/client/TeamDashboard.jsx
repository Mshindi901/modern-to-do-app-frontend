...
function getTeamErrorMessage(error) {
  ...
  const routeIsMissing = typeof responseBody === 'string' && /Cannot (GET|POST|PUT|DELETE) \/api\/team/i.test(responseBody);
  if (error?.response?.status === 404 && routeIsMissing) {
    return 'The deployed backend does not have the team API yet. Deploy the backend branch containing the team routes, then retry.';
  }
...
export default function TeamDashboard() {
  ...
  const loadTeams = async () => {
    try {
      ...
      setNotice({ type: 'error', message: getTeamErrorMessage(error) });
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    fetchTeamList(user?.id)
      ...
      .catch((error) => {
        if (active) setNotice({ type: 'error', message: getTeamErrorMessage(error) });
      })
  ...
  const handleInvite = async (event) => {
    ...
    try {
      ...
      setNotice({ type: 'error', message: error?.message === 'No account was found for that email address.' ? error.message : getTeamErrorMessage(error) });
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return <div className="flex min-h-screen items-center justify-center bg-[#f8f7ff] text-sm text-slate-500">Loading team space...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-[#f8f7ff] text-slate-900 lg:flex">
                  ...
                  <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
                    ...
                    {membersLoading ? <div className="p-8 text-center text-sm text-slate-500">Loading roster...</div> : visibleMembers.length === 0 ? <div className="p-8 text-center text-sm text-slate-500">No roster details available.</div> : visibleMembers.map((member) => (
                      <div key={member.id} className="grid gap-3 border-b border-slate-100 px-4 py-4 last:border-0 sm:grid-cols-[minmax(0,1fr)_140px_160px] sm:items-center sm:gap-4">
                        ...
                        <div>{member.role === 'owner' ? <span className="inline-flex items-center gap-1.5 rounded-md bg-violet-50 px-2 py-1 text-xs font-semibold capitalize text-violet-800"><ShieldCheck size={13} /> Owner</span> : canManageMembers ? <select value={member.role} onChange={(event) => handleRoleChange(member, event.target.value)} className="rounded-md border border-slate-200 bg-white px-2 py-1.5 text-xs capitalize outline-none focus:border-violet-500" aria-label={`Role for ${member.user_id}`}><option value="member">Member</option><option value="admin">Admin</option></select> : <span className="text-sm capitalize text-slate-600">{member.role}</span>}</div>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#241541]/55 p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) setModal(''); }}>
          <section className="w-full max-w-md rounded-xl bg-white p-5 shadow-2xl sm:p-6" role="dialog" aria-modal="true" aria-labelledby="team-modal-title">
            <div className="mb-5 flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase text-violet-800">{modal === 'invite' ? 'Team access' : 'Workspace'}</p><h2 id="team-modal-title" className="mt-1 text-xl font-semibold">{modal === 'create' ? 'Create a team' : modal === 'edit' ? 'Edit team details' : 'Invite a member'}</h2></div><button onClick={() => setModal('')} className="rounded-md p-1.5 text-slate-500 hover:bg-slate-100" aria-label="Close dialog"><X size={18} /></button></div>
            {modal === 'invite' ? <form onSubmit={handleInvite} className="space-y-4"><label className="block text-sm font-medium text-slate-700">Account email<input type="email" value={inviteForm.email} onChange={(event) => setInviteForm({ ...inviteForm, email: event.target.value })} placeholder="name@example.com" className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-violet-500" required /></label><label className="block text-sm font-medium text-slate-700">Role<select value={inviteForm.role} onChange={(event) => setInviteForm({ ...inviteForm, role: event.target.value })} className="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none focus:border-violet-500"><option value="member">Member</option><option value="admin">Admin</option></select></label><p className="text-xs leading-5 text-slate-500">The person must already have a The lazy account. They will be added immediately.</p><div className="flex justify-end gap-2 pt-2"><button type="button" onClick={() => setModal('')} className="rounded-lg px-3 py-2 text-sm text-slate-600 hover:bg-slate-100">Cancel</button><button type="submit" disabled={saving} className="inline-flex items-center gap-2 rounded-lg bg-[#6242c7] px-4 py-2 text-sm font-medium text-white hover:bg-[#5032ae] disabled:opacity-60"><Mail size={15} />{saving ? 'Adding...' : 'Add member'}</button></div></form> : <form onSubmit={handleSaveTeam} className="space-y-4"><label className="block text-sm font-medium text-slate-700">Team name<input value={teamForm.name} onChange={(event) => setTeamForm({ ...teamForm, name: event.target.value })} maxLength={100} className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-violet-500" required /></label><label className="block text-sm font-medium text-slate-700">Description <span className="font-normal text-slate-400">(optional)</span><textarea value={teamForm.description} onChange={(event) => setTeamForm({ ...teamForm, description: event.target.value })} maxLength={1000} rows={4} placeholder="What is this team working on?" className="mt-1.5 w-full resize-y rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-violet-500" /></label><div className="flex justify-end gap-2 pt-2"><button type="button" onClick={() => setModal('')} className="rounded-lg px-3 py-2 text-sm text-slate-600 hover:bg-slate-100">Cancel</button><button type="submit" disabled={saving} className="inline-flex items-center gap-2 rounded-lg bg-[#6242c7] px-4 py-2 text-sm font-medium text-white hover:bg-[#5032ae] disabled:opacity-60"><Check size={15} />{saving ? 'Saving...' : modal === 'create' ? 'Create team' : 'Save changes'}</button></div></form>}
          </section>
        </div>
      )}

      {pendingRemoval && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#241541]/55 p-4">
